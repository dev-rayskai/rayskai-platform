...
export default function RevenueOs(){
  return <SiteShell>
    ...
    <section id="employees" className="mx-auto max-w-7xl px-6 pb-16"><p className="eyebrow">YOUR AI REVENUE TEAM</p><h2 className="section-title mt-5 max-w-2xl">Specialized employees for every patient moment.</h2><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{employees.map(([title,copy])=><article key={title} className="glass rounded-2xl p-6"><span className="text-primary">✦</span><h2 className="mt-6 font-semibold">{title}</h2><p className="mt-3 text-sm leading-6 text-muted">{copy}</p></article>)}</div></section><section className="mx-auto max-w-7xl px-6 pb-16">
