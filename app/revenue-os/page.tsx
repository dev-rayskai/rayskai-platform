...
export default function RevenueOs(){
  return <SiteShell>
    ...
    <section id="employees" data-nav-label="AI Team" className="mx-auto max-w-7xl px-6 pb-16"><p className="eyebrow">YOUR AI REVENUE TEAM</p><h2 className="section-title mt-5 max-w-2xl">Specialized employees for every patient moment.</h2><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{employees.map(([title,copy])=><article key={title} className="glass rounded-2xl p-6"><span className="text-primary">✦</span><h2 className="mt-6 font-semibold">{title}</h2><p className="mt-3 text-sm leading-6 text-muted">{copy}</p></article>)}</div></section><section id="workflow" data-nav-label="Workflow" className="mx-auto max-w-7xl px-6 pb-16">
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">{workflows.map(([title,copy],index)=><article key={title} className="glass rounded-3xl p-7"><span className="text-sm font-semibold text-primary">0{index+1}</span><h2 className="mt-8 text-2xl font-semibold tracking-[-.04em]">{title}</h2><p className="mt-4 leading-7 text-muted">{copy}</p></article>)}</div>
    </section>
    <section id="scale" data-nav-label="Scale" className="mx-auto max-w-7xl px-6 pb-28"><div className="overflow-hidden rounded-3xl border border-primary/20 bg-primary/[.07] p-8 sm:p-12"><p className="eyebrow">BUILT TO SCALE WITH YOUR PRACTICE</p><h2 className="mt-5 max-w-3xl text-4xl font-semibold tracking-[-.055em] sm:text-5xl">Start with revenue recovery. Grow into a complete Revenue OS.</h2><div className="mt-10 grid gap-4 text-sm text-muted sm:grid-cols-3"><p>✓ Multi-location orchestration</p><p>✓ Dedicated AI employees</p><p>✓ Custom integrations & reporting</p></div><a href="/contact" className="mt-10 inline-flex rounded-full bg-primary px-6 py-3.5 font-semibold text-white">Talk to sales</a></div></section>
  </SiteShell>;
}
